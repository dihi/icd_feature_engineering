...
calc_l2 <- function(model_matrix, train_indices, test_indices){
  ...
  for(i in split){
    ...
    set.seed(1)
    time_elapsed_death <- rbind(time_elapsed_death, system.time(model_death <- cv.glmnet(y = model_matrix$death_in_year[subset_indices], x = x_matrix[subset_indices, ], alpha = 0, family = "binomial", maxit = 10000)))
...
if (args[1] == "ahrq"){
  ...
  l2_ahrq_total_results <- calc_l2(ahrq_total_with_death, train_indices, test_indices)
  saveRDS(l2_ahrq_total_results, paste0(args[2], "l2_ahrq_total_results.rds"))
  ...
  l2_ahrq_binary_results <- calc_l2(ahrq_binary_with_death, train_indices, test_indices)
  saveRDS(l2_ahrq_binary_results, paste0(args[2], "l2_ahrq_binary_results.rds"))
...
if (args[1] == "ccs"){
  ...
  l2_ccs_total_results <- calc_l2(ccs_total_with_death, train_indices, test_indices)
  saveRDS(l2_ccs_total_results, paste0(args[2], "l2_ccs_total_results.rds"))
  ...
  l2_ccs_binary_results <- calc_l2(ccs_binary_with_death, train_indices, test_indices)
  saveRDS(l2_ccs_binary_results, paste0(args[2], "l2_ccs_binary_results.rds"))
...
if(args[1] == "trunc"){
  ...
  l2_trunc_total_results <- calc_l2(trunc_total_with_death, train_indices, test_indices)
  saveRDS(l2_trunc_total_results, paste0(args[2], "l2_trunc_total_results.rds"))
  ...
  l2_trunc_binary_results <- calc_l2(trunc_binary_with_death, train_indices, test_indices)
  saveRDS(l2_trunc_binary_results, paste0(args[2], "l2_trunc_binary_results.rds"))
}
...
if(args[1] == "raw"){
  ...
  l2_raw_total_results <- calc_l2(raw_total_with_death, train_indices, test_indices)
  saveRDS(l2_raw_total_results, paste0(args[2],"l2_raw_total_results.rds"))
  ...
  l2_raw_binary_results <- calc_l2(raw_binary_with_death, train_indices, test_indices)
  saveRDS(l2_raw_binary_results, paste0(args[2],"l2_raw_binary_results.rds"))
